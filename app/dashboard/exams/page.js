  ...
  if (!errorMessage && exams.length > 0) {
    content = (
      <div className="grid gap-3 lg:grid-cols-2 lg:gap-4">
        {exams.map((exam) => (
          <Card key={exam.id} className="glass-panel-strong rounded-[1.35rem] py-3 sm:rounded-[2rem] sm:py-4">
            <CardHeader className="space-y-3 border-b border-white/8 px-4 pb-4 sm:px-5 sm:pb-5">
              <StatusBadge status={exam.status} />
              <CardTitle className="break-words text-lg sm:text-2xl">{exam.title}</CardTitle>
            </CardHeader>
